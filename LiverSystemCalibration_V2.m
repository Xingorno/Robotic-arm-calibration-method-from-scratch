...
%%
save 2nd_length_total length_total
save 2nd_phi_total  phi_total
save 2nd_coeff_total  coeff_total
save 2nd_residual_total  residual_total
...
%%
save CALIBRATION_PARA_LENGTH_2230_1  length_corrected
%%
...
%%
save CALIBRATION_PARA_PHI_DEGREE_2230_1  phi_corrected_degree
save CALIBRATION_PARA_PHI_RADIAN_2230_1  phi_corrected_radian
%%
...
%%
save CALIBRATION_PARA_COEFF_DEGREE_2230_1 coeff_corrected_degree
save CALIBRATION_PARA_COEFF_RADIAN_2230_1 coeff_corrected_radian
...
LENGTH = load('CALIBRATION_PARA_LENGTH_2230_1.mat');
LENGTH = LENGTH.length_corrected'

PHI = load('CALIBRATION_PARA_PHI_RADIAN_2230_1.mat');
PHI = PHI.phi_corrected_radian'
% PHI = [PHI 0]

coeff = load('CALIBRATION_PARA_COEFF_RADIAN_2230_1.mat');
...
encoder2_LUT = load('lookup_table_encoder2_new_mean.mat');
encoder5_LUT = load('lookup_table_encoder5_new2_mean.mat');
% THETA = [-15.5566; 7.5586; -1.2305; 1.5820; -0.4395; -5.625; -79.6289]*pi/180;
...
% working space
x_min = min(v_tip2base_corrected(1,:))
x_max = max(v_tip2base_corrected(1,:))
y_min = min(v_tip2base_corrected(2,:))
y_max = max(v_tip2base_corrected(2,:))
z_min = min(v_tip2base_corrected(3,:))
z_max = max(v_tip2base_corrected(3,:))
...
mean(global_abs_value)
std(global_abs_value)
...
table_relative_distance = [];
table_absolute_distance = [];
for i = 165:244
    temp = i - 165;
    table_relative_distance(floor(temp/8)+1, mod(temp,8)+1) = delta_NDI_arm(i);
    table_absolute_distance(floor(temp/8)+1, mod(temp,8)+1) = global_abs_value(i);
end
...
figure (4)
xvalues = {'10','50','90','130','170','210','250','290' };
yvalues = {'10','50','90','130','170'};
h = heatmap(xvalues,yvalues,abs(table_absolute_distance(1:5,:)))
title('Absolute distance (Z=0)')
xlabel('X direction (mm)')
ylabel('Y direction (mm)')
figure (5)
xvalues = {'10','50','90','130','170','210','250','290' };
yvalues = {'10','50','90','130','170'};
h = heatmap(xvalues, yvalues,abs(table_relative_distance(1:5,:)))
title('Relative distance (Z=0)')
xlabel('X direction (mm)')
ylabel('Y direction (mm)')
...
xvalues = {'10','50','90','130','170','210','250','290' };
yvalues = {'10','50','90','130','170'};
h = heatmap(xvalues, yvalues,abs(table_absolute_distance(6:10,:)))
title('Absolute distance (Z=-20 mm)')
xlabel('X direction(mm)')
ylabel('Y direction(mm)')
figure (7)
xvalues = {'10','50','90','130','170','210','250','290' };
yvalues = {'10','50','90','130','170'};
h = heatmap(xvalues, yvalues,abs(table_relative_distance(6:10,:)))
title('Relative distance (Z=-20 mm)')
xlabel('X direction (mm)')
ylabel('Y direction (mm)')
...
vector_absolute_distance = reshape(table_absolute_distance, [],1);
mean(vector_absolute_distance)
std(vector_absolute_distance)

mean(table_abs_mean_relative_distance(165:244))
std(table_abs_mean_relative_distance(165:244))

mean(table_signed_mean_relative_distance(165:244))
std(table_signed_mean_relative_distance(165:244))
...
figure (2)
scatter3(v_NDI_tip2ref_corrected_aligned(1,165:244), v_NDI_tip2ref_corrected_aligned(2,165:244), v_NDI_tip2ref_corrected_aligned(3,165:244),'filled');
hold on
scatter3(v_tip2base_corrected(1,165:244), v_tip2base_corrected(2,165:244), v_tip2base_corrected(3,165:244),'filled');
hold off
legend('Tip in NDI', 'Arm tip')
xlabel('X')
ylabel('Y')
